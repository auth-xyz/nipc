...
void CLIParser::print_help() const {
  ...
  for (const auto& [name, flag] : flags_) {
    size_t length = name.length() + (flag.alias.empty() ? 0 : flag.alias.length() + 4); 
    max_length = std::max(max_length, length);
  }


  for (const auto& [name, arg] : arguments_) {
    size_t length = name.length() + (arg.alias.empty() ? 0 : arg.alias.length() + 4); 
    max_length = std::max(max_length, length);
  }
  ...
  for (const auto& [name, flag] : flags_) {
    ...
    std::cout << std::string(padding, ' ') << ": " << flag.description << " (default: " 
